...
fn initial_magic() -> [u32; 0x412] {
    let mut magic = [0u32; 0x412];
    for (i, word) in magic.iter_mut().enumerate() {
        let p = i * 4;
...
fn init2(magic: &mut [u32; 0x412], args: &mut [u32; 3]) {
    let mut left = args[2];
    let mut right = args[1];
    encrypt(magic, &mut left, &mut right);
    args[2] = left;
    args[1] = right;

    let mut left = args[1];
    let mut right = args[0];
    encrypt(magic, &mut left, &mut right);
    args[1] = left;
    args[0] = right;
    let mut key = [0u8; 8];
    key[..4].copy_from_slice(&args[0].to_le_bytes());
    key[4..].copy_from_slice(&args[1].to_le_bytes());
    update_hashtable(magic, &key);
}

fn init1(gamecode: u32) -> ([u32; 0x412], [u32; 3]) {
    let mut magic = initial_magic();
    let mut args = [gamecode, gamecode >> 1, gamecode << 1];
...
pub(crate) fn transform_secure_area(path: &std::path::Path, option: char) -> io::Result<()> {
    ...
    if option == 'd' {
        ...
        put_word(&mut area, 0, right);
        put_word(&mut area, 4, left);
        if word(&area, 0) != 0x72636e65 || word(&area, 4) != 0x6a624f79 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "secure area decryption check failed",
            ));
        }
        put_word(&mut area, 0, 0xe7ffdeff);
        put_word(&mut area, 4, 0xe7ffdeff);
        for p in (8..0x800).step_by(8) {
            let mut left = word(&area, p + 4);
            let mut right = word(&area, p);
            decrypt(&magic, &mut left, &mut right);
        ...
        put_word(&mut area, 0, 0x72636e65);
        put_word(&mut area, 4, 0x6a624f79);
        let mut left = word(&area, 4);
        let mut right = word(&area, 0);
        encrypt(&magic, &mut left, &mut right);
        put_word(&mut area, 0, right);
        put_word(&mut area, 4, left);
        let (magic2, _) = init1(gamecode);
        let mut left = word(&area, 4);
        let mut right = word(&area, 0);
...
mod tests {
    ...
    fn encrypt_decrypt_round_trip() {
        ...
        let original = (0x12345678, 0x9abcdef0);
